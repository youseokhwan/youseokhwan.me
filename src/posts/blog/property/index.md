...
클래스나 구조체 내부에 있는 변수와 함수는 각각 필드(field)와 메서드(method)로 부른다.

프로퍼티(property)는 객체의 속성을 나타낸다는 점에서는 필드와 유사하나
...
저장 프로퍼티(stored property)는 단순히 값을 저장하는 필드의 개념이다.
...
연산 프로퍼티(computed property)는 호출될 때마다 값을 연산하여 반환하는 프로퍼티이다.
...
타입 프로퍼티(type property)는 각각에 인스턴스에 속하는 값이 아닌 타입 자체에 속하는 값이다.
