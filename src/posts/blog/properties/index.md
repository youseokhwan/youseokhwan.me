...
프로퍼티<sup>Properties</sup>는 객체의 속성을 나타낸다는 점에서는 필드와 유사하나
...
저장 프로퍼티<sup>Stored Properties</sup>는 단순히 값을 저장하는 필드의 개념이다.
...
연산 프로퍼티<sup>Computed Properties</sup>는 호출될 때마다 값을 연산하여 반환하는 프로퍼티이다.
...
`get`, `set` 키워드를 생략하고 `return`만 명시하면 읽기 전용 프로퍼티<sup>Read-Only Properties</sup>를 만들 수 있다.
...
Swift는 프로퍼티의 값이 초기화될 때마다 원하는 동작을 할 수 있도록 옵저버<sup>Property Observers</sup>를 제공한다.
...
타입 프로퍼티<sup>Type Properties</sup>는 각각에 인스턴스에 속하는 값이 아닌 타입 자체에 속하는 값이다.
