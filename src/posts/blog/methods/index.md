...
클래스나 구조체 내부에 있는 함수를 메서드<sup>Methods</sup>라고 한다.

Swift에서는 각 인스턴스에서 실행할 수 있는 인스턴스 메서드와 메모리를 공유하는 타입 메서드로 세분화한다.

## 인스턴스 메서드

인스턴스 메서드<sup>Instance Methods</sup>는 특정 클래스, 구조체의 인스턴스에 속한 메서드이다.
...
타입 메서드<sup>Type Methods</sup>는 타입 자체에 속하는 메서드이다.
